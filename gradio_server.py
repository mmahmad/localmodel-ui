...
#Call Ollama API
desktop_url = "http://localhost:11434/api/generate"
docker_url = "http://ollama:11434/api/generate"

def generate(prompt, context, top_k, top_p, temp):
    r = requests.post(docker_url,
                     json={
                         'model': model,
                         'prompt': prompt,
                         'context': context,
...
block.launch(debug=True, server_name="0.0.0.0", server_port=7860)